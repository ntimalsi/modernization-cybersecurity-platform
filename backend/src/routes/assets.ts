...
export async function assetsRoutes(app: FastifyInstance) {
  ...
  app.get("/assets", async (req) => {
    ...
    const assets = await app.prisma.asset.findMany({ where: { institutionId }, orderBy: { createdAt: "desc" } });
    return { assets };
  });
  ...
  app.post("/assets/:id/baseline", async (req) => {
  ...
  app.post("/assets/:id/current", async (req) => {
    const { id } = req.params as any;
    const data = (req.body as any) ?? {};
    const snap = await app.prisma.configSnapshot.create({
